...
- `Encoding/EncodeQueue.cs` — job store + channel feeding `EncodeQueueHostedService`, which encodes one job at a time via HandBrakeCLI (`--json` stdout is parsed for progress). Job updates fan out to WebSocket subscribers of `/api/encode/queue/ws`. The list is persisted to `<media root>/.mediatryk/queue.json` (via `EncodeQueueStateStore`) on every status change — never on progress ticks; on startup, pending jobs are re-enqueued as Queued, and on graceful shutdown a running job is requeued rather than canceled.
...
- Encodes write to `<dest>.encoding`, then move into place — an in-progress file is never visible through the media API. Canceled/failed encodes delete the `.encoding` file and prune destination directories left empty (never the media root).
